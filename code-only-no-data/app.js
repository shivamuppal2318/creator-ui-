...
function syncRangeToolbar() {
  const activeRange = String(state.filters.range || state.dashboard?.filters?.range || "30");
...
function buildSceneTimeline(post) {
  ...
  if (hasRealTranscriptContent(post) && Array.isArray(post.sceneBreakdown) && post.sceneBreakdown.length) {
    return post.sceneBreakdown;
  }
  if (hasRealTranscriptContent(post) && post.transcript) {
...
function openCompetitorModal(handle) {
  ...
  state.modalReturnTarget = { type: "competitor", id: handle };
  const competitor = (state.dashboard?.competitors || []).find((item) => item.canonicalHandle === handle);
  if (!competitor) return;
  state.activeCompetitorHandle = handle;
...
function openCompetitorReelDetail(postId) {
  ...
  const hasRealTranscript = hasRealTranscriptContent(post);
  ...
  const tags = hasAnalysis
    ...
    : [];
  const visibleScenes = hasRealTranscript
    ? scenes
    : [];
  ...
  $("#competitorDetailTags").innerHTML = tags.length
    ...
    : "";
  $("#competitorDetailScript").innerHTML = hasAnalysis && scriptLines.length
  ...
  toggleHidden("#competitorScriptBlock", !(hasAnalysis && scriptLines.length));
}
...
function renderDashboard(data) {
  ...
  state.filters = {
    ...state.filters,
    range: String(data.filters?.range || state.filters.range || "30"),
  ...
  $("#competitors").innerHTML = data.competitors
    .map(
      (competitor) => `
        <button class="competitor-row" type="button" data-competitor-handle="${escapeHtml(competitor.canonicalHandle || "")}">
          ...
          <div class="competitor-metric positive">
            <strong>${escapeHtml(competitor.monthlyGrowthLabel)}</strong>
            <small>content momentum</small>
          </div>
          <div><span class="post-pill soft">${escapeHtml(competitorHookLabel(competitor))}</span></div>
          <div><span class="post-pill">${escapeHtml(competitor.bestFormat)}</span></div>
          <div class="competitor-idea">${escapeHtml(competitorFastestContent(competitor))}</div>
        </button>
      `,
    )
    .join("") || emptyMarkup("No competitor records available yet.");
  document.querySelectorAll("[data-competitor-handle]").forEach((button) => {
    button.onclick = () => openCompetitorModal(button.dataset.competitorHandle);
  });
  ...
  $("#newsSyncStrip").innerHTML = `
    <span class="sync-badge">Synced ${escapeHtml(data.header.syncLabel.replace("Store updated ", ""))}</span>
    <span class="sync-copy">${escapeHtml(data.dataSources?.news?.label || "Niche-matched stories ready for reel conversion")}</span>
    <button class="mini-action" id="refreshNewsAction" type="button">Refresh now</button>
  `;
  $("#featuredNewsPanel").innerHTML = visibleFeaturedStory
    ? `
      ...
      <div class="news-tags">
        <span class="news-tag">${escapeHtml(visibleFeaturedStory.nicheFitLabel)}</span>
        ${(visibleFeaturedStory.matchedSignals || []).slice(0, 2).map((tag) => `<span class="news-tag">Fits ${escapeHtml(tag)}</span>`).join("")}
      </div>
      <div class="featured-news-actions">
        <button class="primary-news-action" data-story-id="${escapeHtml(visibleFeaturedStory.id || "")}" data-story-prompt="${escapeHtml(buildNewsPrompt(visibleFeaturedStory))}" type="button">Turn this into a reel</button>
        <button class="ghost-button story-ghost" data-story-id="${escapeHtml(visibleFeaturedStory.id || "")}" data-story-prompt="${escapeHtml(buildNewsPrompt(visibleFeaturedStory))}" type="button">Build hooks + structure</button>
        ${visibleFeaturedStory.url ? `<a class="ghost-button story-ghost" href="${escapeHtml(visibleFeaturedStory.url)}" target="_blank" rel="noreferrer">Read source</a>` : ""}
      </div>
    `
      ...
      (story) => `
        <article class="news-card">
          ...
          <div class="news-topline">
            ...
            <span class="news-fit">${escapeHtml(story.nicheFitLabel || `Fit ${story.nicheScore || 0}`)}</span>
          </div>
          ...
          ${
            story.matchedSignals?.length
              ? `<div class="news-tags">
                  ${story.matchedSignals
                    .slice(0, 3)
                    .map((tag) => `<span class="news-tag">Fits ${escapeHtml(tag)}</span>`)
                    .join("")}
                </div>`
              : ""
          }
          <p class="news-angle">${escapeHtml(story.reelBlueprint?.angle || "")}</p>
          <div class="featured-news-actions">
            <button class="ghost-button story-ghost" data-story-id="${escapeHtml(story.id || "")}" data-story-prompt="${escapeHtml(buildNewsPrompt(story))}">Turn into reel</button>
            ${story.url ? `<a class="ghost-button story-ghost" href="${escapeHtml(story.url)}" target="_blank" rel="noreferrer">Read source</a>` : ""}
          </div>
        </article>
      `,
    )
